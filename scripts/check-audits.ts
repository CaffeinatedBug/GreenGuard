...
async function checkAudits() {
  try {
    ...
    audits?.forEach((audit, i) => {
      ...
      console.log(`   Energy: ${audit.iot_logs?.energy_kwh} kWh`);
      console.log(`   Supplier: ${audit.iot_logs?.suppliers?.name || 'Unknown'}`);
      console.log(`   Reasoning: ${audit.agent_reasoning?.slice(0, 150)}...`);
      console.log(`   Has agent_logs: ${audit.agent_logs ? 'Yes (' + JSON.parse(audit.agent_logs as any).length + ' logs)' : 'No'}`);
